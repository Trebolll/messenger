...
func (r *MessageRepository) SendMessage(message *model.Message) error {
	query := `
		...
		SELECT m.id, m.chat_id, m.sender_id, u.username, COALESCE(u.avatar_url,''),
		       COALESCE(u.rating,0), m.content, m.created_at, m.read_at, m.edited_at, m.likes, m.dislikes
		FROM inserted_msg m
...
func (r *MessageRepository) EditMessage(messageID, senderID uuid.UUID, content string) (*model.Message, error) {
	query := `
		...
		SELECT u.id, u.chat_id, u.sender_id, usr.username, COALESCE(usr.avatar_url,''),
		       COALESCE(usr.rating,0), u.content, u.created_at, u.read_at, u.edited_at, u.likes, u.dislikes
		FROM updated u
