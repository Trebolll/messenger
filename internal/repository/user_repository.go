...
func (r *UserRepository) GetByEmail(email string) (*model.User, error) {
	u := new(model.User)
	err := r.db.QueryRow("SELECT id, username FROM users WHERE email = $1", email).Scan(&u.ID, &u.Username)
	if err != nil {
		return nil, err
	}
	return u, nil
...
func (r *UserRepository) GetById(id int) (*model.User, error) {
	u := new(model.User)
	err := r.db.QueryRow("SELECT id, username FROM users WHERE id = $1", id).Scan(&u.ID, &u.Username)
	if err != nil {
		return nil, err
	}
	return u, nil
